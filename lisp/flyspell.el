...
(defun flyspell-attempt-correction (beg end poss func)
  "Apply FUNC to all points in the region BEG to END.
Return t if any those result in a possible replacement suggested by
ispell in POSS.  Otherwise the change is undone.

FUNC is called with arguments (BEG END POSS) to determine if the
buffer is actually corrected or not.

`flyspell-attempt-correction' exists to make the typical legwork
mindless.  Therefore, don't bother with save-excursion and the undo
list, etc.  It's all been done for you.  like

FUNC can throw the tag `done' with t if it succeeds, or nil if it
doesn't.  Returning those values the normal way also means the same
thing."
  (let ((modified-p (buffer-modified-p))
	(here (point))
	success)
    (unwind-protect
	(setq success (save-excursion
			(catch 'done
			  (funcall func beg end poss))))
      (goto-char here)
      (unless success
	;; make it look like we deleted the word, and then
	;; inserted its corrected form
...
This function is meant to be added to `flyspell-incorrect-hook'."
  (when (consp poss)
	...
	(while (< (point) end)
	  (transpose-chars 1)
	  (if (member (buffer-substring beg end) (third poss))
	      (throw 'done t))
	  (transpose-chars -1)
	  (forward-char)))))))

(custom-add-option 'flyspell-incorrect-hook
		   'flyspell-maybe-correct-transposition)

(defun flyspell-maybe-correct-doubling (beg end poss)
  "For each doubled character in the region BEG to END, remove one.
Return t if any those result in a possible replacement suggested by
ispell in POSS.  Otherwise the change is undone.
  ...
  (when (consp poss)
	...
	(let ((last (char-after beg))
	  ...
	  (while (< (point) end)
	    ...
	    (when (char-equal this last)
	      (delete-char 1)
	      (if (member (buffer-substring beg (1- end)) (third poss))
		  (throw 'done t))
	      ;; undo
	      (insert-char this))
	    (setq last this))))))))
