...
(and window-system
	 ...
	 (defun how-faces-differ (face1 face2 &optional frame)
	   ...
	   (let ((difference nil))
		 (or (equal (face-foreground face1 frame) (face-foreground face2 frame))
			 (setq difference (cons 'foreground difference)))
		 (or (equal (face-background face1 frame) (face-background face2 frame))
			 (setq difference (cons 'background difference)))
		 (or (equal (face-font face1 frame) (face-font face2 frame))
			 (setq difference (cons 'font difference)))
		 (or (eq (face-underline-p face1 frame) (face-underline-p face2 frame))
	 ...
	 (defun find-not-underlined-version-of-face (base)
	   ...
	   (if (or (null base) (not (face-underline-p base)))
		   base
		 (let ((all-faces (face-list))
			   (retval    nil))
		   (while all-faces
	 ...
	 (defun face-underline-char (N)
	   ...
	   (let ((face (get-text-property N 'face)))
		 (put-text-property N (1+ N) 'face (find-underlined-version-of-face face))))
	 
	 (defun face-ununderline-char (N)
	   "Make the character not appear underlined"
	   (interactive "d")
	   (let ((face (get-text-property N 'face)))
		 (put-text-property N (1+ N) 'face (find-not-underlined-version-of-face face))))
	   ...
	   (if (< start end)
		   (let ((mid  (1+ start))
			 ...
			 (while (and (<= mid end)
			   ...
			   (setq mid (1+ mid)))
			 (put-text-property start mid 'face (find-underlined-version-of-face face))
	   ...
	   (if (< start end)
		   (let ((mid  (1+ start))
			 ...
			 (while (and (<= mid end)
						 (equal (get-text-property mid 'face) face))
			   (setq mid (1+ mid)))
			 (put-text-property start mid 'face (find-not-underlined-version-of-face face))
	 ...
	 (defun fix-underlining-of-buffer ()
	   ...
	   (save-excursion
		 (let ((start) (end))
		   (goto-char (point-min))
		   (setq start (re-search-forward "_\b\\|\b_" (point-max) t))
		   (if start
			   (progn
				 (goto-char (point-max))
				 (setq end (re-search-backward "_\b\\|\b_" (1+ start) t))
				 (if (and start end)
					 (fix-underlining-of-region start end)))))))
...
which specify the range to operate on."
	 ...
	 (add-hook 'find-file-hook 'fix-underlining-of-buffer))
