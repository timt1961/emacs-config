...
;; $Id: ftelnet.el,v 1.3 1995/03/15 00:29:30 friedman Exp $
...
;;;###autoload
(defun ftelnet (input-args &optional prefix)
...
variable."
  ...
  (let* ((process-connection-type ftelnet-process-connection-type)
    ...
    (cond
     ...
     ;; like ftelnet-pop-to-buffer) to avoid extra string consing via multiple
     ;; concats.
     ((and (numberp prefix)
           (let ((bufname (concat buffer-name "<" prefix ">")))
             (and (comint-check-proc bufname)
                  (ftelnet-pop-to-buffer bufname)))))
     (t
      (cond
       ((numberp prefix)
      ...
      (ftelnet-pop-to-buffer buffer-name)
      (comint-exec (current-buffer) buffer-name ftelnet-program nil args)
