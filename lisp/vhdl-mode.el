...
;; Version: $Revision: 2.56.1.1 $
;; Last Modified: $Date: 1995/03/08 07:13:51 $
;; Keywords: VHDL editing major-mode

;; Copyright (C) 1992, 1993, 1994 Barry A. Warsaw
;; Copyright (C) 1992, 1993, 1994 Free Software Foundation, Inc.
;; Copyright (C) 1994, 1995 Rodney J. Whitby

;; This vhdl-mode package is free software; you can redistribute it
;; and/or modify it under the terms of the GNU General Public License
;; as published by the Free Software Foundation; either version 2, or
;; (at your option) any later version.

;; This vhdl-mode package is distributed in the hope that it will be
...
;; assumes you will use .hdl extensions for your VHDL source:
;;
;; (autoload 'vhdl-mode   "vhdl-mode" "VHDL Editing Mode" t)
;; (setq auto-mode-alist
;;   (append '(("\\.hdl$"  . vhdl-mode)   ; to edit VHDL code
;;            ) auto-mode-alist))

;; If you would like to join the vhdl-mode-announce distribution list
;; (to which all new major stable versions are sent), or the
;; vhdl-mode-victims distribution list (to which all patches and beta
;; versions are sent), then please send mail to the address listed at
;; the top of this file.  Note that anything sent to the
;; vhdl-mode-announce distribution list will also be posted to
;; gnu.emacs.sources and comp.lang.vhdl newsgroups, so if you read
;; these newsgroups you don't need to be on the distribution list (and
;; I'd prefer that you weren't, to reduce the cost of my outgoing
;; email volume).

;; Many, many thanks go out to all the folks on the vhdl-mode-victims
;; list.  Without their patience, testing, insight, and encouragement
;; vhdl-mode.el would be a far inferior package.

;; LCD Archive Entry:
;; vhdl-mode.el|Rodney J. Whitby|rwhitby@asc.corp.mot.com
;; |Major mode for editing VHDL code
;; |$Date: 1995/03/08 07:13:51 $|$Revision: 2.56.1.1 $|
...
(defun vhdl-populate-syntax-table (table)
...
(if vhdl-mode-syntax-table
  ...
  (vhdl-populate-syntax-table vhdl-mode-syntax-table)
...
(defun vhdl-common-init ()
  ;; Common initializations for vhdl-mode.
  ...
  (setq paragraph-start (concat "^$\\|" page-delimiter)
	...
	indent-line-function 'vhdl-indent-line
	comment-column 32
	comment-start-skip "-- *")
...
keyword."
  (cond
   ...
   ((and (looking-at "i")
	 (save-excursion
	   ;; Skip backward over first sexp (will skip an
	   ;; identifier, a case expression, a procedure
	   ;; interface list, or a function return expression.
	   (backward-sexp)
	   (let (foundp)
  ...
  (let ((count (or count 1))
	...
	(if (setq end-vec (vhdl-corresponding-end lim))
		...
		(if (or (and column
		  ...
		  ;; previous sexp.
		  (if (not (save-match-data (looking-at "else")))
...
stops due to beginning or end of buffer." 
  ...
  (let ((last-forward (point))
	...
	(setq last-forward (point))
	(while (and (not foundp)
		    (search-forward "is" last-backward t))
	  (if (or (= (preceding-char) ?_)
		  (save-match-data
...
(defun vhdl-skip-case-alternative (&optional lim)
  ...
  (let ((lim (or lim (point-max)))
    ...
    (while (and (< (point) lim)
		(not donep))
      (if (and (re-search-forward vhdl-case-alternative-key lim 'move)
	       (save-match-data
		 (not (vhdl-in-literal)))
...
(defconst vhdl-version "$Revision: 2.56.1.1 $"
