...
(defconst postscript-font-lock-comment "%" "start of postscript comment")
...
(if ps-mode-syntax-table
  ...
  (let ((i 0))
    ...
    (while (< i 256)
      (or (= (char-syntax i ) ?w)	; xemacs uses ps-mode-syntax-table
					; but since it's copied from the 
					; default, it works
...
with no args, if that value is non-nil."
  ...
  (setq font-lock-keywords postscript-font-lock-keywords)
  (make-local-variable 'font-lock-comment-start-regexp)
  (setq font-lock-comment-start-regexp postscript-font-lock-comment)
  ;
...
(if (null ps-mode-map)
    (progn
      ...
      ; xemacs only: (set-keymap-name ps-mode-map 'ps-mode-map)
