...

(if (or (not (boundp 'emacs-minor-version)) (< emacs-minor-version 29))
...
(defun font-lock-menu-submit-feedback ()
  ...
  (let ((reporter-prompt-for-summary-p t))
    (reporter-submit-bug-report "simon@gnu.ai.mit.edu" "font-lock-menu 1.00"
				nil nil nil "Hi Si.,")))
...
(defun font-lock-menu-all-keywords ()
  (let* ((keyword (or (nth 0 font-lock-defaults)
	 ...
	 (keywords (and keyword
			(all-completions (symbol-name keyword) obarray))))
    (mapcar 'intern (sort keywords 'string-lessp))))
