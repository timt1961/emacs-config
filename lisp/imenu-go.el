...
(defun imenu---in-alist (str alist)
  ...
  (let (elt head tail res)
    (while alist
      ...
      (if (string= str head) (setq alist nil res (cdr elt))
	(if (and (listp tail) (setq res (imenu---in-alist str tail)))
	    (setq alist nil))))
      res))
...
`imenu-go--back'. Can be bound both to mouse and keyboard event."
  ...
  (let ((pos (imenu---can-find-all-buffers default)))
    ...
    (if pos (progn (switch-to-buffer (car pos)) (goto-char (nth 1 pos)))
      (find-tag default))))
