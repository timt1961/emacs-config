;;; If you are using GNU Emacs 19.28 or earlier, I recommend the following
;;; patch to bytecomp.el.  Otherwise, byte compiling swiftex.el with a default
;;; major mode other than `emacs-lisp-mode' might give strange errors.  This
;;; bug has been reported and will be fixed in versions of GNU Emacs > 19.28.
;;;
;;; Make a backup of your bytecomp.el, and then modify it by adding the
;;; definition for the function `get-buffer-create-protect' given below.  Then
;;; change the four calls to `get-buffer-create' to calls to
;;; `get-buffer-create-protect'.  Recompile bytecomp.el.  Now you can compile
;;; swiftex.el safely.
;;;
...
(require 'tex-mode)
(require 'outline)
(defconst stx-version "$Revision: 1.19 $"
...
it is treated specially.  In swifTeX mode, it is treated
just like the other sectioning commands.  In docTeX mode,
\"\\endinput\" will be an outline heading only if it begins
in the first column of the buffer.  There is no reason for
it to occur as an executed macro behind a comment in the
documentation text, and when \"\endinput\" occurs with
preceding whitespace in the code text, doc.sty considers it
to be part of a macro being defined, and does not obey it,
so in this case also we do not want it to be an outline
heading.

`outline-regexp' is prefixed by \"^\" when it is used by the
outline commands, and it is always enclosed in parentheses,
so we don't need to use them here to surround our `rx-or's."
    ...
    (setq outline-regexp
	  (rx-catenate-or
	   ...
	   (concat stx-line-start
		   ...
		   "\\)"
		   "\\>")))
    ;; STEP TWO -- define `stx-outline-level' and `byte-compile' it.  
    ;; We want to make `stx-outline-level' as fast as possible, so we want to
    ;; avoid evaluating conditionals which are going to be the same throughout
    ;; a buffer's life.
    (byte-compile
     (eval 
      (` 
...
must be called at the beginning of a header line."))
	 ...
	 (if (looking-at (concat stx-line-start ;    ""  or "%*"
	     ...
	     (, (if doctex
		    ;; length minus length of "%^^A"
		    '(- (- (match-end 0) (match-beginning 0)) 4)
		  ;; length minus length of "%"
	   ...
	   (let ((count 0)
		 ...
		 (, (if (and doctex endinput-entry)
			(` (if (looking-at (concat rx-backslash
			     ...
			     (error "`stx-outline-level' falling through!")))
...
\\{swiftex-mode-map}"
  ...
  (setq swiftex-dtx-mode-map (make-sparse-keymap))
...
Keep point over the same text as when the function is called."
  (let ((oldpoint (point)) 
	...
	(error "Point is neither in nor adjacent to a word!")
      ;; It would make the most sense to cons `oldpoint' onto the head of
      ;; `buffer-undo-list' before the first `insert'.  But this does not
      ;; work because `insert' somehow senses it is the first command in this
      ;; function to alter the buffer and adds an undo boundary (nil) to the
      ;; list before adding its own entry.  We therefore have to let `insert'
      ;; add the boundary and its own entry, and then splice in `oldpoint'
      ;; between the first and second elements.
      (insert after)
      (setcdr buffer-undo-list (cons oldpoint buffer-undo-list))
      (goto-char start)
      (insert before)
...
\\{doctex-mode-map}"
  ...
  (stx-set-vars)
  (setq doctex-dtx-mode-map (make-sparse-keymap))
