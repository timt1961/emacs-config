...
(defun mail-hist-define-keys ()
  ...
  (local-set-key "\M-r" 'mail-hist-previous-matching-input)
  (local-set-key "\M-s" 'mail-hist-next-matching-input))
...
(defvar mail-hist-search-history nil)

(defun mail-hist-previous-matching-input (repeat header regexp)
  "Insert previous contents of this mail header or body that match REGEXP.
Moves back through the history of sent mail messages.  Each header has
its own independent history, as does the body of the message.

The history only contains the contents of outgoing messages, not
received mail."
  (interactive (list (eq last-command 'mail-hist-input-access)
		     (or (mail-hist-current-header-name) "body")
		     (read-from-minibuffer
		      (format "Previous %s matching (regexp): "
			      (if (mail-hist-current-header-name)
				  (concat (mail-hist-current-header-name)
					  " header")
				"message body"))
		      nil nil nil 'mail-hist-search-history nil t)))
  (setq header (downcase header))
  (let* ((ring (cdr (assoc header mail-hist-header-ring-alist)))
         (len (ring-length ring))
	 (start mail-hist-access-count))
    (if repeat
        (setq mail-hist-access-count
              (ring-plus1 mail-hist-access-count len))
      (setq mail-hist-access-count 0))
    (if (null ring)
      ...
      (if (ring-empty-p ring)
          (error "\"%s\" ring is empty." header)
	(while (not (string-match regexp
				  (ring-ref ring mail-hist-access-count)))
	  (setq mail-hist-access-count
		(ring-plus1 mail-hist-access-count len))
	  (if (= mail-hist-access-count start)
	      (error "No earlier matching %s"
		     (if (equal header "body")
			 "message body"
		       (concat (capitalize header) ": header")))))
        (and repeat
             (delete-region (car mail-hist-last-bounds)
                            (cdr mail-hist-last-bounds)))
        (let ((start (point)))
...
(defun mail-hist-next-matching-input (repeat header regexp)
  "Insert next contents of this mail header or body that match REGEXP.
Moves back through the history of sent mail messages.  Each header has
its own independent history, as does the body of the message.

Although you can do so, it does not make much sense to call this
without having called `mail-hist-previous-header' \
(\\[mail-hist-previous-header]) or `mail-hist-previous-matching-header' \
(\\[mail-hist-previous-matching-header]) first.
