...
;; and the latter to all (future) buffers.  (Buffers loaded when the
;; global mode is activated are not affected; this may change in the
;; future.)
;;
...
;;
;; Unfortunately, there is no way to turn deactivate the CWarn highlighting.
;; The reason is that CWarn use font-lock mode for the highlighting, and
;; there is no way to remove font-lock keywords.
...
;; Future extensions:
;;
;; * Find a way to turn it off (check if cwarn is enabled inside the
;;   match functions, or restart font-lock).
;; * Let global CWarn mode apply to all open buffers.
;; * K&R declarations.
;; * Highlight assignent operators in the test expression of a for
;;   statement.
;; * Highlight trailing ";" in C++ class headers.
...
C++ are included."
  ...
  (if (and cwarn-mode
	   font-lock-mode)
      ;; Ensure that our font-lock hook is called.
      (font-lock-mode 1)
    (error "CWarn mode can't be turned off."))
  (if cwarn-mode
...
Existing buffers are not affected (this may change in the future.)"
  (interactive "P")
...
(defun cwarn-is-enabled (mode feature)
...
(defun cwarn-font-lock-hook-function ()
  ...
  (when (or cwarn-mode
	...
	(font-lock-add-keywords
	 nil
	 '((cwarn-reference-matcher (0 font-lock-warning-face)))))))
