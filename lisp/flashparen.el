...
;; $Date: 1995/03/24 19:47:33 $|$Revision: 1.3 $|~/misc/flashparen.el.gz|

;; $Id: flashparen.el,v 1.3 1995/03/24 19:47:33 friedman Exp $
...
(defun flash-matching-mode (&optional prefix)
  ...
  ;; Make sure flash-matching-char is last on post-command-hook.
  (let ((h (memq 'flash-matching-char post-command-hook)))
    (cond ((null h)
           (add-hook 'post-command-hook 'flash-matching-char 'append))
          ((cdr h)
           (remove-hook 'post-command-hook 'flash-matching-char)
           (add-hook 'post-command-hook 'flash-matching-char 'append))))
         ...
         (cond
          ((or (and (numberp cho)
           ...
           (let ((parse-sexp-ignore-comments t)
             ...
             ;; open/close syntax class but with no defined matching character.
             (and match-point
                  (matching-paren ch)
                  (not (= (char-after match-point) (matching-paren ch)))
